...
void usage(const char* argv0) {
    std::printf(
        ...
        "  --chat           Format prompt in Qwen chat template (<|im_start|>user...)\n"
...
std::string format_chat(const std::string& user_prompt) {
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        const char* a = argv[i];
        if (!std::strcmp(a, "--dir")) dir_path = next(a);
    ...
    if (dir_path.empty()) {
        ...
        if (env_dir) {
            ...
            std::fprintf(stderr, "error: --dir PATH is required (or set BROLM_QWEN35_DIR)\n");
            usage(argv[0]);
            return 1;
        }
    }
    ...
    try {
        ...
        q35::Tokenizer tok = q35::Tokenizer::load(vocab_p.string(), merges_p.string());
        ...
        q35::Qwen35Config::Text cfg = full_cfg.text;
        ...
        // Multimodal checkpoints store text weights under "model.language_model." or "language_model."
        ...
        auto run_generation = [&](const std::string& input_text) {
            std::string prompt_formatted = (chat_mode ? format_chat(input_text) : input_text);
