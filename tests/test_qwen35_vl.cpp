// Qwen3.5-VL top-level driver integration test.
//
...
//     the vision tower and the text backbone. Loads via VLM::load_from_directory
//     and generates a few text-only tokens (no images supplied), exercising
//     the prefill -> sample -> decode loop and the forward_embeds entry
//     point.
//
// (b) Real-checkpoint — gated on BROLM_QWEN35_DIR. Loads the real 0.8B VLM
//     checkpoint, builds a synthetic 224x224 gray image (constant 0.5), runs
//     a ChatML prompt containing one image_pad triple, and decodes up to 16
//     greedy tokens. Asserts the pipeline completes without NaN/crash and
//     that the prompt's image_pad expansion matches num_image_tokens. We do
//     NOT assert any specific output string — numerical parity with HF is
...
// ─── safetensors fixture builder (same pattern as test_qwen35_text.cpp) ───
...
// Mirror the GPT-2 byte->unicode mapping used inside qwen::Tokenizer so the
// synthetic vocab.json contains every byte-fallback unit the BPE encoder will
// emit on plain ASCII / ISO-8859-ish inputs.
...
// JSON-escape a string (only the characters that vocab.json's parser
// understands: ", \, control chars).
...
// Minimal config.json matching the structure expected by Qwen35Config::load,
// configured to a tiny shape suitable for the synthetic test.
//
// We don't try to exhaustively cover every HF field — just what brolm parses.
void write_synth_config(const std::filesystem::path& path,
                        const q35::Qwen35Config& cfg) {
    std::ofstream f(path);
    f << "{";
    ...
    f << "\"linear_num_key_heads\":"   << cfg.text.linear_num_key_heads << ",";
    f << "\"linear_num_value_heads\":" << cfg.text.linear_num_value_heads << ",";
    f << "\"linear_key_head_dim\":"    << cfg.text.linear_key_head_dim << ",";
    f << "\"linear_value_head_dim\":"  << cfg.text.linear_value_head_dim << ",";
    f << "\"linear_conv_kernel_dim\":" << cfg.text.linear_conv_kernel_dim << ",";
    f << "\"full_attention_interval\":" << cfg.text.full_attention_interval << ",";
    f << "\"layer_types\":[";
    for (std::size_t i = 0; i < cfg.text.layer_types.size(); ++i) {
        if (i) f << ",";
        f << "\""
          << (cfg.text.layer_types[i] == q35::LayerType::Full
                  ? "full_attention" : "linear_attention")
          << "\"";
    }
    f << "],";
    f << "\"rope_parameters\":{\"rope_type\":\"default\",\"rope_theta\":1.0e7,"
         "\"mrope_section\":[" << cfg.text.rope.mrope_section[0] << ","
         << cfg.text.rope.mrope_section[1] << ","
         << cfg.text.rope.mrope_section[2] << "],"
         "\"partial_rotary_factor\":" << cfg.text.rope.partial_rotary_factor
...
// Build a synthetic safetensors covering BOTH model.visual.* and
// model.language_model.*. The latter prefix is what TextModel expects by
// default; the former is what VisionTower expects.
void write_synth_safetensors(const std::filesystem::path& path,
    ...
    const int H_lin   = tc.linear_num_value_heads;
    const int D_k     = tc.linear_key_head_dim;
    const int D_v     = tc.linear_value_head_dim;
    const int lin_kv  = H_lin * D_k;
    const int lin_vv  = H_lin * D_v;
    const int lin_qkv = 3 * lin_kv;
    const int conv_ch = lin_qkv;
    const int conv_kd = tc.linear_conv_kernel_dim;
    ...
    for (int i = 0; i < tc.num_hidden_layers; ++i) {
        ...
        if (tc.layer_types[static_cast<std::size_t>(i)] == q35::LayerType::Full) {
...
// IDs (0..255) the synthetic vocab maps fit; the chosen image_token_id /
// vision_start_token_id / vision_end_token_id all fit too.
q35::Qwen35Config make_synth_config() {
    q35::Qwen35Config cfg;
    ...
    tc.linear_key_head_dim    = 16;
    tc.linear_value_head_dim  = 16;
    tc.linear_conv_kernel_dim = 4;
    tc.rope.rope_theta            = 1.0e7f;
    tc.rope.partial_rotary_factor = 0.5f;        // rotary_dim = 16
    tc.rope.mrope_section         = {3, 3, 2};
...
void run_synthetic() {
    ...
    const fs::path dir = fs::temp_directory_path() / "brolm_qwen35_vl_synth";
    std::error_code ec;
    ...
    write_synth_safetensors(
        dir / "model.safetensors-00001-of-00001.safetensors", cfg);
    ...
    try {
        ...
        // through the GPT-2 byte-fallback alphabet; with no merges every byte
        // becomes one id.
        const std::string prompt = "Hi";
        std::vector<q35::ImageInput> no_images;
        ...
        // Grammar-constrained decode. Ids past the byte alphabet have no
        // text here, so the mask leaves only bytes the grammar can take.
        ...
        // Complete after "42" with no stop token allowed: the decode ends early.
        CHECK(text_of(g_ids) == "42");
        CHECK(!exact.is_accepted());  // the caller's grammar is never advanced
        ...
        // Clearing the grammar restores the unconstrained decode.
        ...
        std::fprintf(stderr, "qwen35_vl synthetic threw: %s\n", e.what());
        ++g_failures;
    }
...
void run_real_checkpoint() {
    ...
    try {
        ...
        // Synthetic 224x224 mid-gray image.
        ...
        // Sanity: prompt tokenization contains exactly one image_pad.
        ...
        std::printf("vl real: first generated ids:");
        const int show = std::min(5, static_cast<int>(out_ids.size()));
        for (int i = 0; i < show; ++i) {
            std::string one = vlm.tokenizer().decode({out_ids[i]});
            std::printf(" [%d=%d \"%s\"]", i, out_ids[i], one.c_str());
        }
        std::printf("\n");
        ...
        std::fprintf(stderr, "qwen35_vl real-checkpoint threw: %s\n", e.what());
        ++g_failures;
    }
}
...
int main() {
    ...
    run_synthetic();
    run_real_checkpoint();
    if (g_failures == 0) std::printf("qwen35_vl: OK\n");
    else std::fprintf(stderr, "qwen35_vl: %d failure(s)\n", g_failures);
