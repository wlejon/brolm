...
// Qwen3-VL tokenizer.
//
// Qwen3-VL uses the standard Qwen3 vocab (vocab.json + merges.txt, 151936
// entries) — the same byte-level BPE as text-only Qwen3 — plus a handful of
// multimodal "added tokens" that live ABOVE the base vocab range (in
// tokenizer_config.json's `added_tokens_decoder`, not in vocab.json itself):
// <|vision_start|>, <|vision_end|>, <|image_pad|>, <|video_pad|>.
// <|endoftext|>, <|im_start|>, <|im_end|> are already inside the base vocab
// and auto-registered by the inner qwen::Tokenizer loader.
//
...
class Tokenizer {
public:
    // Load a Qwen3-VL checkpoint's `vocab.json` + `merges.txt`. The four
    // vision/video specials (see file comment) are registered at their
    // documented ids in addition to whatever is already in
